	...
	*/
	homomorphism(symbol_mapping){
		...
		for(var source=0; source<self.states.length; source++){
			for(var symbol in self.states[source]){
				const homomorphism = symbol_mapping.get(symbol);
				assert(homomorphism);
				for(var j=0; j<self.states[source][symbol].length; j++){
					self.states[source][symbol].forEach(function(target){
						if (homomorphism.length === 0) {
							// Add an epsilon-transition between source and target
							addTransition(source, ε, target);
						}else{
							var intermediate = source;
							for (var i = 0; i < homomorphism.length - 1; i++) {
								var current = new_states.length;
								new_states[current] = {};
								addTransition(intermediate, homomorphism[i], current);
								intermediate = current;
							}
							addTransition(intermediate, homomorphism[homomorphism.length - 1], target);
						}
					});
				}
			}
