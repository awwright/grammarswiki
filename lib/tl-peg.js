...
class tl_peg extends tl {
	...
	/* Translate the given ABNF document to .peg */
	static translateABNF(abnf){
		...
		return supplied + Object.keys(abnfcore).filter(k => unrecognized.indexOf(k) !== -1).map(v => map(abnfcore[v])).join('\n') + '\n';
		...
		function map(node){
			if(node instanceof abnflib.rulelist){
				...
				return '[' + node.nums.map(function(v){
					// FIXME filter out codepoints above ASCII for now
					// Until the UTF-32 to UTF-16 homomorphism is implemented
					if (v.lower >= 0x80){
						return '';
					}
					if(typeof v.upper === 'number'){
						if(v.upper >= 0x80){
							return `\\x${v.lower.toString(16).padStart(2, '0')}}-\\x7F`;
						}
