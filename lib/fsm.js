...
class FSM {
	...
	static fromNFA(nfa){
		// Run through nfa.get to expand epsilon-transitions
		const initial = nfa.getAllArray(nfa.initial);

		function toKey(set){ return [...set].sort().join(';'); }
		...
		const old_states_to_new_state_id_map = new Map([[toKey(initial), 0]]);
		// The new states
		...
		for (var i = 0; i < new_state_to_old_states_map.length; i++) {
			...
			const state_alphabet = nfa.alphabetArray(cross_product);
			...
			for (const symbol of state_alphabet) {
				...
				const nextStates = nfa.nextStateArray(cross_product, symbol);
				// console.log(i, symbol, nextStates);
				// Generate a key name for this cross-product
				const nextKey = toKey(nextStates);
			...
			const is_final_state = cross_product.some(v => nfa.finals.has(v));
			// console.log(i, cross_product, final_state_i);
			if (is_final_state) new_final_states.add(i);
		}
