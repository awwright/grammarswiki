...
class FSM {
	constructor(map, initial, finals){
		...
		if (this.map[this.initial] === undefined) throw new Error('`initial` not a state in `map`');
	...
	*/
	alphabet() {
		const self = this;
		const abc = new Set;
		self.map.forEach(function (transitions) {
	...
	*/
	accepts(input){
		...
		for(var i=0; i<input.length; i++){
			...
			if(!self.map[state] || self.map[state][symbol]===undefined){
	...
	toString(){
		...
		self.map.forEach(function(transitions, state){
	...
	*/
	static parallel(fsms, final) {
		...
		for (var i = 0; i < cross_product_list.length; i++) {
			...
			const state = state_i.map((i, j) => fsms[j].map[i]);
	...
	*/
	static remap(initial, alphabet, next, final) {
		...
		const cross_product_list = [initial];
		// A handy mapping of each combination of states to a reduced state
		const cross_product_map = new Map([[initial.join(';'), 0]]);
		// The new states
		const combination_states = [];
		const combination_states_final = new Set;
		...
		for (var i = 0; i < cross_product_list.length; i++) {
			// Compute the symbols used by each state
			const cross_product = cross_product_list[i];
			...
			for (const symbol of state_alphabet) {
				...
				nextStates.forEach(v => { assert.equal(typeof v[0], 'number'); assert.equal(typeof v[1], 'number'); });
				// Generate a key name for this cross-product
				...
				if (nextId !== undefined) {
					...
					// Create a new state
					transitions[symbol] = cross_product_list.length;
					assert.equal(typeof cross_product_list.length, 'number');
					cross_product_map.set(nextKey, cross_product_list.length);
					cross_product_list.push(nextStates);
				}
			}
			...
			if (final_state_i) combination_states_final.add(i);
		}
	...
	static concatenate(){
		...
		function alphabet(states){
			const state_fsms = states.map((v) => [v[0], fsms[v[0]].map[v[1]]]).filter(v => v[1]);
		...
		function next(current, symbol) {
			...
			current.forEach(function (val) {
				...
				var fsm = fsms[fsm_i];
				if (!fsm) return;
				const transitions = fsm.map[substate];
	...
	star() {
		...
		function next(current, symbol) {
			...
			current.forEach(function (val) {
				...
				if (transitions && transitions[symbol]!==undefined) {
					...
					if (!states.some(v => (v[1] === transitions[symbol]))){
						states.push([fsm_i, transitions[symbol]]);
					}
					// If this is a final state, also follow from the initial
					if (self.finals.has(transitions[symbol]) && !states.some(v => (v[1] === self.initial))) {
						states.push([fsm_i, self.initial]);
					}
				}
			});
			return states;
		...
		function final(state_list) {
			return state_list.some(function (state_val) {
				return self.finals.has(state_val[1]);
			});
		}
	...
	*/
	reverse(){
		...
		const reverse = self.map.map(_ => ({}));
		self.map.forEach(function (symbols, left) {
	...
	islive(state){
		...
		for(var i=0; i<reachable.length; i++){
			...
			if(self.map[current]){
				Object.keys(self.map[current]).forEach(function(symbol){
					const next = self.map[current][symbol];
	...
	*strings(){
		...
		const live = self.map.map((_, state)=>self.islive(state));
		...
		const alphabets = self.map.map(function(s){
		...
		for(var i=0; i<strings.length; i++){
			...
			if(!self.map[state]) continue;
			for (var j = 0; j < alphabet.length; j++){
				const symbol = alphabet[j];
				const nstate = self.map[state][symbol];
	...
	*/
	cardinality(){
		...
		function get_num_strings(state){
			...
			if(self.islive(state)){
				...
				if(self.map[state]){
					self.map[state].forEach(function(transition){
	...
	derive(input){
		...
		return new FSM(self.map, state, self.finals);
	}
}
