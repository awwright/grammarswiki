...
@Suite("NFA Tests") struct NFATests {
	...
	@Test("Initialization with specific states") func Initialization2() {
		...
		//		["b": Set([0])],
		//	];
		//	let nfa = SymbolNFA<Character>(states: transitions, epsilon: [[], []], initials: [0], finals: [1])
	...
	@Test("Initialization with verbatim input") func Initialization3() {
		let nfa = SymbolNFA(verbatim: "abc")
	...
	@Test("Contains for valid input") func Initialization4() {
		let nfa = SymbolNFA<Character>(verbatim: "abc")
	...
	@Test("Next States Calculation") func Initialization5() {
		let nfa = SymbolNFA<Character>(states: [["a": [1]], ["b": [0]]], epsilon: [[], []], initial: 0, finals: [1])
		#expect(nfa.nextStates(states: [0], symbol: "a") == [1])
	...
	@Test("Union of SymbolNFAs") func Initialization7() {
		let nfa1 = SymbolNFA(verbatim: "abc")
		let nfa2 = SymbolNFA(verbatim: "def")
		let unionSymbolNFA = nfa1.union(nfa2)
		#expect(unionSymbolNFA.contains("abc") && unionSymbolNFA.contains("def"))
	}

	@Test("Intersection of SymbolNFAs") func Initialization8() {
		let nfa1 = SymbolNFA(verbatim: "abc")
		let nfa2 = SymbolNFA(verbatim: "abc")
		let intersectSymbolNFA = nfa1.intersection(nfa2)
		#expect(intersectSymbolNFA.contains("abc"))

		let nfa3 = SymbolNFA(verbatim: "def")
	...
	@Test("Symmetric Difference") func Initialization9() {
		let nfa1 = SymbolNFA(["aaa", "bbb"])
		let nfa2 = SymbolNFA(["bbb", "ddd"])
	...
	func test_concatenate() {
		let epsilon = SymbolNFA<Character>.concatenate([])
		#expect(epsilon.contains(""))

		let dfa1 = SymbolNFA<Character>(["a", "b"])
		let single = SymbolNFA.concatenate([dfa1]);
		#expect(single.contains("a"))

		let dfa2 = SymbolNFA<Character>(["x", "y"])
		...
		let triple = SymbolNFA.concatenate([dfa1, dfa1, dfa1]);
		#expect(triple.contains("aba"))

		let range = SymbolNFA.concatenate([dfa1.optional(), dfa1, dfa1.optional()]);
...
//		#expect(array.count == 3)
	...
	func test_repeating_int() {
		let original = SymbolNFA<Character>(["a", "b"])
	...
	@Test("Set Operations") func Initialization0() {
		var nfa = SymbolNFA<Character>(["abc", "def"])
		#expect(nfa.contains("abc"))
		#expect(nfa.contains("def"))

		nfa.formIntersection(SymbolNFA(verbatim: "abc"))
		#expect(nfa.contains("abc"))
		#expect(!nfa.contains("def"))

		nfa.formSymmetricDifference(SymbolNFA(verbatim: "def"))
		#expect(nfa.contains("abc"))
		#expect(nfa.contains("def"))

		nfa.formSymmetricDifference(SymbolNFA(verbatim: "abc"))
		#expect(!nfa.contains("abc"))
		#expect(nfa.contains("def"))
	}
	...
	@Test("Simple Homomorphism - Identity Mapping") func Test01() {
		...
		let newSymbolNFA: SymbolNFA<Character> = nfa.homomorphism(mapping: mapping)

		#expect(newSymbolNFA.contains("abc"))
		#expect(!newSymbolNFA.contains("ab"))
	}
	...
	@Test("Homomorphism - Symbol Replacement") func Test02() {
		...
		#expect(newSymbolNFA.contains("xyz"))
		#expect(!newSymbolNFA.contains("abc"))
		#expect(!newSymbolNFA.contains("xy"))
	}

	@Test("Homomorphism - Symbol to Multiple Symbols") func Test03() {
		let nfa = SymbolNFA<Character>(verbatim: "a")
		let mapping: [(String, String)] = [("a", "bb")]
		let newSymbolNFA: SymbolNFA<Character> = nfa.homomorphism(mapping: mapping)

		#expect(newSymbolNFA.contains("bb"))
		#expect(!newSymbolNFA.contains("a"))
		#expect(!newSymbolNFA.contains("b"))
	}

	@Test("Homomorphism - Multiple Symbols to One Symbol") func Test04() {
		let nfa = SymbolNFA<Character>(verbatim: "ab")
		let mapping: [(String, String)] = [("a", "x"), ("b", "x")]
		let newSymbolNFA: SymbolNFA<Character> = nfa.homomorphism(mapping: mapping)

		#expect(newSymbolNFA.contains("xx"))
		#expect(!newSymbolNFA.contains("x"))
	}

	@Test("Homomorphism - Heterogneous types") func Test05() {
		let language = SymbolNFA<Character>(["ab", "ba"]);
		let mapping: [(String, Array<UInt8>)] = [("a", [1]), ("b", [2])]
		let translation: SymbolNFA<UInt8> = language.homomorphism(mapping: mapping)
	...
	@Test("Homomorphism - No Mapping for Some Symbols") func Test07() {
		let language = SymbolNFA<Character>(verbatim: "abc")
		let mapping: [(String, String)] = [("a", "x"), ("b", "b"), ("c", "c")]
		let translation: SymbolNFA<Character> = language.homomorphism(mapping: mapping)
...
//			(DFA(["c"]), DFA(["c", "\\x63"])),
//		]
//		let translation = SymbolNFA(["abc"]).homomorphism(mapping: mapping)
