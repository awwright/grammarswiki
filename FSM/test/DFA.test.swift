...
@Suite("DFA Tests") struct DFATests {
	...
	func testDFAFromVerbatim() {
		let dfa = SymbolDFA<Character>(verbatim: "abc")
	...
	func testDFAFromList() {
		let dfa2: SymbolDFA<UInt8> = [ [0x30],  [0x31],  [0x32, 0x32],  [0x32, 0x33],  [0x33, 0x32],  [0x33, 0x33] ];
	...
	func testEqualsOperator() {
		let a = SymbolDFA(["abc", "def"]);
		let b = SymbolDFA(["def", "abc"]);
		#expect(a == b)
	}
	...
	func test_nextState_symbol() {
		...
		let dfa_bool = SymbolDFA<Bool>(verbatim: [true, false, true])
	...
	func test_nextState_string() {
		...
		let dfa = SymbolDFA<Bool>(verbatim: [true, false, true])
	...
	@Test("Greedy match")
	func test_match() {
		let dfa = SymbolDFA<Character>(["a", "ab", "xy"])
		...
		let dfa2 = SymbolDFA<Character>(["", "abc"])
		#expect(dfa2.match("")! == ("", ""))
	...
	func test_equivalent() throws {
		let dfa = SymbolDFA<Character>(["a", "aa", "aaa", "aaaa"]).concatenate(SymbolDFA<Character>(["b", "bb", "bbb", "bbbb"])).concatenate(SymbolDFA<Character>(["a"]).star()).minimized()
	...
	func test_minimized() {
		...
		let providedDictionary = ABNFBuiltins<SymbolDFA<UInt8>>.dictionary
	...
	func test_mapTransitions() {
		...
		let dfa1 = SymbolDFA<UInt8>([ [0x61], [0x61, 0x62], [0x63, 0x64] ]);
		let dfa2: SymbolDFA<Character> = dfa1.mapSymbols({
			symbol in
	...
	@Test("Union of DFAs")
	func testDFAUnion() {
		let dfa1 = SymbolDFA<Character>(verbatim: "a")
		let dfa2 = SymbolDFA<Character>(verbatim: "b")
	...
	func testDFAIntersection() {
		let dfa1 = SymbolDFA<Character>(["a", "b"])
		let dfa2 = SymbolDFA<Character>(["b", "c"])
	...
	func testDFASymmetricDifference() {
		let dfa1 = SymbolDFA<Character>(["a", "b", "ab"])
		let dfa2 = SymbolDFA<Character>(verbatim: "ab")
	...
	func test_concatenate() {
		...
		let triple = SymbolDFA.concatenate([dfa1, dfa1, dfa1]);
		#expect(triple.contains("aba"))

		let range = SymbolDFA.concatenate([dfa1.optional(), dfa1, dfa1.optional()]);
	...
	func test_repeating_int() {
		let original = SymbolDFA<Character>(["a", "b"])
	...
	func test_cloverleaf() async throws {
		...
		let S = SymbolDFA<UInt8>.symbol(0x1);
		let T = SymbolDFA<UInt8>.symbol(0x2);
		let U = SymbolDFA<UInt8>.symbol(0x3);
		let expression: SymbolDFA<UInt8> = ( (R).union( (S).concatenate(U.star()).concatenate(T) ) ).star().concatenate(S).concatenate(U.star())
		let expression_fsm: SymbolDFA<UInt8> = expression.toPattern()
	...
	func test_paths_2() {
		...
		#expect(paths == [
			[],
			[SymbolDFA<Character>.PathIterator.Segment(source: 0, index: 0, symbol: "x", target: 1)],
		])
	}
	...
	func test_paths_filter_0() {
		...
		func filter(iterator: SymbolDFA<Character>.PathIterator, path: SymbolDFA<Character>.PathIterator.Path) -> Bool {
			return path.count < 3;
		}
		var array: Array<SymbolDFA<Character>.PathIterator.Path> = [];
		for path in dfa.pathIterator(filter: filter) {
			array.append(path);
		}
		#expect(array == [
			[],
			[SymbolDFA<Character>.PathIterator.Segment(source: 0, index: 0, symbol: "x", target: 0)],
			[SymbolDFA<Character>.PathIterator.Segment(source: 0, index: 0, symbol: "x", target: 0), SymbolDFA<Character>.PathIterator.Segment(source: 0, index: 0, symbol: "x", target: 0)]
		]);
	}
	...
	func test_paths_filter_2() {
		...
		#expect(paths == [
			[],
			[SymbolDFA<Character>.PathIterator.Segment(source: 0, index: 0, symbol: "x", target: 1)],
			[SymbolDFA<Character>.PathIterator.Segment(source: 0, index: 0, symbol: "x", target: 1), SymbolDFA<Character>.PathIterator.Segment(source: 1, index: 0, symbol: "y", target: 2)],
	...
	func testIteratorProtocol1() {
		let dfa = SymbolDFA<Character>(verbatim: "")
		var values: [String] = []
		for string in dfa {
	...
	func testIteratorProtocol3() {
		let dfa = SymbolDFA<Character>(["bc", "a", "abcdefg", "ab", ""])
		var values: [String] = []
		for string in dfa {
			values.append(String(string))
		}
	...
	func test_nextStates_DFA() {
		let dfa = SymbolDFA(["abcdefghi"]);
		let pattern = SymbolDFA(["abc"])
			.concatenate(SymbolDFA(["d"]).optional())
			.concatenate(SymbolDFA(["e"]).optional())
			.concatenate(SymbolDFA(["f"]).optional());
		#expect(dfa.nextStates(initial: 0, input: pattern) == [3, 4, 5, 6]);
	}

	@Test("nextStates by DFA #2")
	func test_nextStates_DFA_2() {
		let dfa = SymbolDFA(["101001000100"]);
		// Follow any number of 0's, then a 1
		let pattern = SymbolDFA(["0"]).star().concatenate(SymbolDFA(["1"]));
	...
	@Suite("toPattern") struct DFATests_toPattern {
		@Test("empty")
		func test_empty() {
			let dfa: SymbolDFA<UInt8> = SymbolDFA([]);
		...
		@Test("character")
		func test_char() {
			let dfa: SymbolDFA<UInt8> = SymbolDFA([ [0x30] ]);
			#expect(Array(dfa).count == 1)
			let pattern: SimpleRegex<UInt8> = dfa.toPattern()
		...
		func test_plus() {
			...
			let dfa: SymbolDFA<UInt8> = SymbolDFA([ [0x30] ]).plus().minimized();
			let pattern: SimpleRegex<UInt8> = dfa.toPattern()
		...
		func test_star() {
			...
			// Is this something that can be fixed in .star()?
			let dfa: SymbolDFA<UInt8> = SymbolDFA([ [0x30] ]).star().minimized();
			let pattern: SimpleRegex<UInt8> = dfa.toPattern()
			#expect(pattern.description == "30*")
		}
	}
	...
	@Suite("alphabet/alphabetPattern") struct DFATests_alphabet {
		func alphabetPartitionsByContext<T>(_ val: SymbolDFA<T>) -> Set<Set<T>> {
			// An alternate way of calculating partitions that is less magic:
			var parts: Dictionary<Set<[SymbolDFA<T>]>, Set<T>> = [:];
		...
		func testAlphabetPartitionsEqual<T>(_ val: SymbolDFA<T>) -> Bool {
			true
//			return alphabetPartitionsByContext(val) == val.alphabetPartitions
		...
		@Test("epsilon") func epsilon() async throws {
			let dfa: SymbolDFA<UInt8> = SymbolDFA([ [] ]).minimized();
			#expect(dfa.alphabet == [])
			#expect(testAlphabetPartitionsEqual(dfa))
		}
		@Test("single") func single() async throws {
			let dfa: SymbolDFA<UInt8> = SymbolDFA([ [0x30] ]).minimized();
			#expect(dfa.alphabet == [0x30])
			#expect(testAlphabetPartitionsEqual(dfa))
		}
		@Test("union") func union() async throws {
			let dfa: SymbolDFA<UInt8> = SymbolDFA([ [0x30], [0x31], [0x32] ]).minimized();
		...
		@Test("intersection") func intersection() async throws {
			let dfa: SymbolDFA<UInt8> = SymbolDFA([ [0x30],  [0x31],  [0x32],  [0x33], [0x30, 0x33] ]).minimized();
			#expect(dfa.alphabet == [0x30, 0x31, 0x32, 0x33])
			#expect(testAlphabetPartitionsEqual(dfa))
		}
		@Test("concatenation 1") func concatenation() async throws {
			let dfa: SymbolDFA<UInt8> = SymbolDFA([ [0x30],  [0x31],  [0x32, 0x32],  [0x33, 0x33] ]).minimized();
			#expect(dfa.alphabet == [0x30, 0x31, 0x32, 0x33])
			#expect(testAlphabetPartitionsEqual(dfa))
		}
		@Test("concatenation 2") func concatenation_2() async throws {
			let dfa: SymbolDFA<UInt8> = SymbolDFA([ [0x30],  [0x31],  [0x32, 0x32],  [0x32, 0x33],  [0x33, 0x32],  [0x33, 0x33] ]).minimized();
			#expect(dfa.alphabet == [0x30, 0x31, 0x32, 0x33])
			#expect(testAlphabetPartitionsEqual(dfa))
		}
	}
...
@Suite("SymbolClassDFA<ClosedRangeAlphabet>") struct SymbolClassDFA_ClosedRangeAlpuabet_Tests {
	@Suite("SymbolClassDFA<ClosedRangeAlphabet>") struct DFATests {
		@Test("union") func union() async throws {
			let dfa1 = RangeDFA<Int>(states: [[[0...4]: 1], [:]], initial: 0, finals: [1])
			#expect(dfa1.contains([0]))
			let dfa2 = RangeDFA<Int>(states: [[[4...9]: 1], [:]], initial: 0, finals: [1])
		...
		@Test("concatenation") func concatenation() async throws {
			let dfa1 = RangeDFA<Int>(states: [[[1...9]: 1], [:]], initial: 0, finals: [1])
			let dfa2 = RangeDFA<Int>(states: [[[0...9]: 0]], initial: 0, finals: [0])
		...
		@Test("intersection") func intersection() async throws {
			let dfa1 = RangeDFA<Int>(states: [[[0...1]: 1], [:]], initial: 0, finals: [1])
			let dfa2 = RangeDFA<Int>(states: [[[1...2]: 1], [:]], initial: 0, finals: [1])
