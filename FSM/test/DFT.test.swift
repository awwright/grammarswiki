...
@Suite("DFT Tests") struct DFTTests {
	...
	func test_top() async throws {
		...
		let dft = SymbolDFT<Character>(top: SymbolDFA<Character>(["", "0", "1", "2"]).star())
		#expect(dft.map("") == [])
		#expect(dft.map("1") == [])
	...
	func test_bottom() async throws {
		...
		let dft = SymbolDFT<Character>(bottom: SymbolDFA<Character>(["", "0", "1", "2"]).star())
		#expect(dft.map("") == [])
		#expect(dft.map("1") == ["1"])
		#expect(dft.map("x") == nil)
		#expect(dft.isEquivalent("00", "00") == true)
	...
	@Test("optional")
	func test_optional() {
		let dfa1 = SymbolDFT(top: SymbolDFA<Character>(["a", "b"]))
